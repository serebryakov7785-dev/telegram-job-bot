...
def is_valid_email(email: str) -> bool:
    ...
    # 5. Проверка популярных доменов Узбекистана
    uz_domains = ['.uz', '.com', '.ru', '.net', '.org']
    if not any(domain.endswith(ext) for ext in uz_domains):
        # Можно закомментировать, если не нужна проверка доменов
        pass
