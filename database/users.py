...
import logging
import time
from .core import execute_query, hash_password, clear_user_state

# ================= КЭШИРОВАНИЕ =================
_user_cache: Dict[int, Tuple[float, Optional[Dict[str, Any]]]] = {}
CACHE_TTL = 60  # Время жизни кэша в секундах
_seekers_cache: Dict[Tuple[int, int], Tuple[float, List[Dict[str, Any]]]] = {}
...
def get_user_by_id(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if user:
            logging.debug(f"Найден соискатель: {user.get('full_name', 'Unknown')} (ID: {user_id})")
        ...
        if user:
            logging.debug(f"Найден работодатель: {user.get('company_name', 'Unknown')} (ID: {user_id})")
            _user_cache[user_id] = (time.time(), user)
            return user.copy()
        
        # Если не нашли - это НЕ ошибка, пользователь может войти с нового аккаунта
        logging.debug(f"Пользователь с Telegram ID {user_id} не найден - может войти с нового аккаунта")
        _user_cache[user_id] = (time.time(), None)
        return None
    except Exception as e:
        logging.error(f"Ошибка получения пользователя по ID {user_id}: {e}", exc_info=True)
        return None
...
def get_user_by_credentials(identifier: str) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    try:
        ...
        """, (identifier_clean, identifier_clean), fetchone=True)
        ...
        logging.warning(f"Пользователь с идентификатором '{identifier_clean}' не найден")
        return None, ''
    except Exception as e:
        logging.error(f"Ошибка поиска пользователя по учетным данным {identifier}: {e}", exc_info=True)
        return None, ''

def update_telegram_id(old_telegram_id: int, new_telegram_id: int, role: str, user_db_id: int) -> bool:
    ...
    try:
        ...
        try:
            result = execute_query(
                f"UPDATE {table} SET telegram_id = ?, last_login = CURRENT_TIMESTAMP WHERE id = ?",
                (new_telegram_id, user_db_id),
                suppress_error=True
            )
        except Exception as e:
            if "no such column: last_login" in str(e):
                logging.warning(f"Колонка last_login отсутствует в {table}, обновляем только telegram_id")
                result = execute_query(
                    f"UPDATE {table} SET telegram_id = ? WHERE id = ?",
                    (new_telegram_id, user_db_id)
                )
            else:
                raise e
        ...
        if result > 0:
            ...
            logging.info(f"Telegram ID обновлен: {old_telegram_id} -> {new_telegram_id} ({role})")
            invalidate_user_cache(old_telegram_id)
            invalidate_user_cache(new_telegram_id)
            return True
        else:
            logging.warning(f"Не удалось обновить Telegram ID для {role} с ID {user_db_id}")
            return False
            
    except sqlite3.IntegrityError as e:
        logging.error(f"Ошибка целостности при обновлении Telegram ID: {e}", exc_info=True)
        return False
    except Exception as e:
...
# ================= ОБНОВЛЕНИЕ ДАННЫХ =================
def update_seeker_profile(telegram_id: int, **updates) -> bool:
    ...
    try:
        allowed_fields = {
            'profession', 'skills', 'experience', 'education', 'languages',
            'full_name', 'age', 'phone', 'email', 'status', 'city'
        }
        ...
        query = f"UPDATE job_seekers SET {', '.join(set_parts)} WHERE telegram_id = ?"
...
def update_employer_profile(telegram_id: int, **updates) -> bool:
    ...
    try:
        allowed_fields = {
            'company_name', 'contact_person', 'description', 'business_activity',
            'phone', 'email', 'city'
        }
        ...
        query = f"UPDATE employers SET {', '.join(set_parts)} WHERE telegram_id = ?"
        result = execute_query(query, tuple(values))
...
def get_all_seekers(limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
    """Получение всех соискателей с пагинацией"""
    # Проверка кэша
    ...
    try:
        result = execute_query(
            "SELECT * FROM job_seekers ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?",
            (limit, offset),
            fetchall=True
        )
...
def delete_seeker_account(telegram_id: int) -> bool:
    ...
    try:
        ...
        result = execute_query(
            "DELETE FROM job_seekers WHERE telegram_id = ?",
            (telegram_id,)
        )
...
def delete_employer_account(telegram_id: int) -> bool:
    ...
    try:
        clear_user_state(telegram_id)
        result = execute_query(
            "DELETE FROM employers WHERE telegram_id = ?",
            (telegram_id,)
        )
