...
def debug_command(message):
    ...
    debug_info = f"🛠️ *Информация для отладки*\n\n"
    debug_info += f"🆔 Ваш Telegram ID: `{user_id}`\n"
    
    if user_data:
        if 'full_name' in user_data:
            debug_info += f"👤 *Тип:* Соискатель\n"
...
@bot.message_handler(func=lambda m: m.text in ['✏️ Изменить', '➕ Добавить', '↩️ Назад в настройки'])
...
def check_rate_limit(message):
    ...
    if len(user_requests[user_id]) >= RATE_LIMIT:
        ...
        try:
            bot.send_message(user_id, f"⏳ Вы отправляете сообщения слишком часто. Пожалуйста, подождите {MUTE_DURATION} секунд.")
        except Exception as e:
...
def process_all_messages(message):
    ...
    try:
        ...
        if user_state and user_state.get('step') in ['profession', 'education', 'languages', 'experience', 'skills']:
            step = user_state['step']
            if step == 'profession':
                profile_handlers.process_profession(message)
            elif step == 'education':
                profile_handlers.process_education(message)
            elif step == 'languages':
                profile_handlers.process_languages(message)
        ...
        if user_state and user_state.get('step') == 'enter_new_value' and user_state.get('action') == 'edit_seeker_field':
