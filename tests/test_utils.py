...
@pytest.mark.parametrize("phone,expected", [
    ...
    ("123901234567", False),     # Length 12, operator 90 inside, but prefix wrong (kills startswith mutant)
...
@pytest.mark.parametrize("phone,expected", [
    ...
    ("8(90)1234567", "8(90)1234567"),    # Starts with 8, len 10, has formatting (kills mutant)
    ("9(90)1234567", "9901234567"),    # Len 10, NOT starts with 8 -> should be cleaned (kills startswith mutant)
...
def test_extract_email_domain():
    assert utils.extract_email_domain("test@example.com") == "example.com"
...
def test_validate_password():
    ...
    assert utils.validate_password("Str0ng!8")[0] is True  # Exactly 8 chars (boundary check)
    assert utils.validate_password("StrongP@ss1" + "RandomChars")[0] is True  # Long password (21 chars)
    ...
    assert utils.validate_password("A" * 97 + "Ab1!")[0] is False  # 101 chars (Too long)
...
def test_validate_captcha():
    ...
    assert utils.validate_captcha("пять", 5) is True
    assert utils.validate_captcha("test", "test") is True
    assert utils.validate_captcha("wrong", 5) is False
    assert utils.validate_captcha(None, 5) is False
...
def test_cancel_request():
    ...
    assert utils.cancel_request(None) is False
    assert utils.cancel_request("🏠") is True  # Emoji check (kills mutant removing exact match)
...
def test_sanitize_input():
    ...
    assert len(utils.sanitize_input("a" * 600)) == 500  # Max length check (kills mutant)
...
def test_validate_name():
    ...
    assert utils.validate_name(None)[0] is False
...
def test_safe_execute():
    ...
    # Error case
    def fail_func(): raise ValueError("Boom")
