...
def test_migration_add_city_to_employers(test_db):
    ...
    test_db.execute("CREATE TABLE employers (id INTEGER PRIMARY KEY, telegram_id INTEGER UNIQUE NOT NULL)")
    test_db.commit()
...
def test_init_database_exception_handling(caplog):
    ...
    with patch('database.schema.execute_query', side_effect=sqlite3.Error("Test DB Error")):
