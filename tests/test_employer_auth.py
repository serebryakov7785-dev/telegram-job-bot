...
class TestEmployerAuth:
    ...
    def test_process_employer_name_valid(self, handler, message):
    ...
    def test_process_employer_name_invalid(self, handler, message):
        ...
        with patch('database.get_user_state', return_value=user_state):
            ...
            assert "Название компании слишком короткое" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_employer_phone_valid(self, handler, message):
        ...
        with patch('database.get_user_state', return_value={'step': 'phone', 'registration_data': {}}), \
    ...
    def test_process_employer_email_valid(self, handler, message):
        ...
        with patch('database.get_user_state', return_value={'step': 'email', 'registration_data': {}}), \
    ...
    def test_process_business_activity_success(self, handler, message):
    ...
    def test_process_business_activity_invalid(self, handler, message):
        ...
        with patch('database.get_user_state', return_value=user_state):
            handler.process_business_activity(message)
            handler.bot.send_message.assert_called()
            assert "Род деятельности слишком короткий" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_employer_phone_duplicate_seeker(self, handler, message):
        ...
        message.text = "901234567"
        with patch('database.get_user_state', return_value={'step': 'phone'}), \
             patch('utils.is_valid_uzbek_phone', return_value=True), \
             patch('utils.format_phone', return_value="+998901234567"), \
    ...
    def test_process_employer_phone_cancel(self, handler, message):
        ...
        message.text = "❌ Отмена"
        with patch('utils.cancel_request', return_value=True), \
             patch('database.get_user_state', return_value={'step': 'phone'}), \
             patch.object(handler, 'cancel_employer_registration') as mock_cancel:
            handler.process_employer_phone(message)
            mock_cancel.assert_called_once()
    ...
    def test_process_employer_email_cancel(self, handler, message):
        ...
        message.text = "❌ Отмена"
        with patch('utils.cancel_request', return_value=True), \
             patch('database.get_user_state', return_value={'step': 'email'}), \
             patch.object(handler, 'cancel_employer_registration') as mock_cancel:
            handler.process_employer_email(message)
            mock_cancel.assert_called_once()
    ...
    def test_process_business_activity_cancel(self, handler, message):
        ...
        message.text = "❌ Отмена"
        with patch('utils.cancel_request', return_value=True), \
             patch('database.get_user_state', return_value={'step': 'business_activity'}), \
             patch.object(handler, 'cancel_employer_registration') as mock_cancel:
            handler.process_business_activity(message)
            mock_cancel.assert_called_once()