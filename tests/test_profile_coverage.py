...
class TestProfileCoverage:
    ...
    def test_process_profession_specific_cancel(self, handler, message):
        ...
        with patch('utils.cancel_request', return_value=True), \
            ...
            assert "можете заполнить профиль позже" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_education_skip(self, handler, message):
        ...
        with patch('database.get_user_state', return_value=user_state), \
            ...
            assert user_state['profile_data']['education'] == 'Не указано'
    ...
    def test_process_language_selection_skip(self, handler, message):
        ...
        message.text = "Пропустить"
        user_state = {'step': 'language_selection', 'role': 'seeker', 'profile_data': {}, 'temp_languages': []}
        ...
        with patch('database.get_user_state', return_value=user_state), \
            ...
            assert user_state['profile_data']['languages'] == 'Не указаны'
    ...
    def test_process_experience_skip(self, handler, message):
        ...
        with patch('database.get_user_state', return_value=user_state), \
            ...
            assert user_state['profile_data']['experience'] == 'Нет опыта'
    ...
    def test_process_skills_skip(self, handler, message):
        ...
        with patch('database.get_user_state', return_value=user_state), \
             patch.object(handler, 'save_profile_data') as mock_save, \
             patch.object(handler, 'finish_profile_setup') as mock_finish:
            handler.process_skills(message)
            assert user_state['profile_data']['skills'] == 'Не указаны'
    ...
    def test_process_language_selection_skip_retry(self, handler, message):
        ...
        user_state = {'step': 'language_selection', 'temp_languages': [{'name': 'En', 'level': 'B2'}]}
        with patch('database.get_user_state', return_value=user_state), \
             patch.object(handler, 'process_language_selection') as mock_retry:
    ...
    def test_save_profile_data_employer_fail(self, handler, message):
        ...
        with patch('database.get_user_by_id', return_value={'company_name': 'Co'}), \
    ...
    def test_process_education_profanity(self, handler, message):
        ...
        with patch('database.get_user_state', return_value={'step': 'education'}), \
    ...
    def test_process_profession_specific_profanity(self, handler, message):
        message.text = "badword"
        with patch('database.get_user_state', return_value={'step': 'profession_specific'}), \
             patch('utils.contains_profanity', return_value=True):
            handler.process_profession_specific(message)
            handler.bot.send_message.assert_called()
            assert "недопустимые слова" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_experience_profanity(self, handler, message):
        message.text = "badword"
        with patch('database.get_user_state', return_value={'step': 'experience'}), \
             patch('utils.contains_profanity', return_value=True):
            handler.process_experience(message)
            handler.bot.send_message.assert_called()
            assert "недопустимые слова" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_skills_profanity(self, handler, message):
        message.text = "badword"
        with patch('database.get_user_state', return_value={'step': 'skills'}), \
             patch('utils.contains_profanity', return_value=True):
            handler.process_skills(message)
            handler.bot.send_message.assert_called()
            assert "недопустимые слова" in handler.bot.send_message.call_args[0][1]
    ...
    def test_process_language_custom_name_short(self, handler, message):
        message.text = "A"
        with patch('database.get_user_state', return_value={'step': 'language_custom_name'}):
            handler.process_language_custom_name(message)
            handler.bot.send_message.assert_called()
    ...
    def test_save_profile_data_employer_success(self, handler, message):
        ...
        user_state = {
            'role': 'employer',
            'profile_data': {'profession': 'IT', 'education': 'High', 'experience': '5y', 'skills': 'Python'}
        }
        employer_data = {'company_name': 'Co', 'description': 'Desc'}

        with patch('database.get_user_by_id', return_value=employer_data), \
             patch('database.update_employer_profile', return_value=True) as mock_update:
