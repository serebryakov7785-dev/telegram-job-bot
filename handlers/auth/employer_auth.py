...
from handlers.profile import PROFESSION_SPHERES
import logging
...
class EmployerAuth:
    ...
    def process_business_activity(self, message):
        ...
        if not user_state or step not in ['business_activity', 'business_activity_custom']:
            self.bot.send_message(message.chat.id, "❌ Сессия истекла!", reply_markup=keyboards.main_menu())
            return
        ...
        if step == 'business_activity':
            ...
            if len(business_activity) < 2:
                self.bot.send_message(message.chat.id, "❌ Род деятельности слишком короткий!\nВведите род деятельности:",
                                      reply_markup=keyboards.cancel_keyboard())
                return
