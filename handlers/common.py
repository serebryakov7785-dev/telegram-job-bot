...
class CommonHandlers:
    ...
    def handle_support(self, message):
        ...
        self.bot.send_message(
            ...
            "📞 *Поддержка*\n\n"
            "🕒 Работаем: 24/7\n"
            "👤 Telegram: @support_jobs_bot\n"
            "📧 Email: support@jobs.uz\n\n"
    ...
    def handle_start_chat(self, call):
        ...
        try:
            ...
            self.bot.send_message(
                ...
                f"Для выхода нажмите кнопку '❌ Завершить чат'.",
                parse_mode='Markdown',
                reply_markup=keyboards.stop_chat_keyboard()
            )
            self.bot.answer_callback_query(call.id)
        except Exception as e:
            logging.error(f"Error starting chat: {e}", exc_info=True)
    ...
    def handle_chat_message(self, message):
        ...
        try:
            ...
            self.bot.send_message(user_id, "✅ Сообщение отправлено")
        except Exception as e:
            logging.error(f"Failed to send chat message: {e}", exc_info=True)
            self.bot.send_message(user_id, "❌ Не удалось отправить сообщение. Возможно, пользователь заблокировал бота.")

    def handle_stop_chat(self, message):
        """Завершение чата"""
        user_id = message.from_user.id
        database.clear_user_state(user_id)
