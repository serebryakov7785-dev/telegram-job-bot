...
class StepHandlers:
    ...
    def handle_steps(self, message):
        ...
        if message.text == "⬅️ Назад" and step in ['city_selection', 'vacancy_profession', 'vacancy_language_level']:
            pass
        ...
        if step.startswith('vacancy_') or step.startswith('edit_vacancy_'):
    ...
    def handle_steps_with_auth_handlers(self, message, step, role, user_state):  # noqa: C901
        ...
        if step in login_steps_map:
            ...
            if hasattr(self.auth_handlers, 'login') and hasattr(self.auth_handlers.login, method_name):
        ...
        elif step == 'enter_new_value' and user_state.get('action') == 'edit_seeker_field':
    ...
    def handle_seeker_steps_fallback(self, message, step, user_state):
        ...
        try:
            ...
            logging.error(f"❌ Fallback ошибка обработки шага соискателя {step}: {e}", exc_info=True)
    ...
    def handle_employer_steps_fallback(self, message, step, user_state):
        ...
        try:
            ...
            logging.error(f"❌ Fallback ошибка обработки шага работодателя {step}: {e}", exc_info=True)
    ...
    def handle_recovery_step_fallback(self, message):
        ...
        try:
            ...
            logging.error(f"❌ Fallback ошибка обработки восстановления: {e}", exc_info=True)

        return False

    def cancel_current_step(self, user_id, chat_id):
        ...
        elif step in ['support_bug_report', 'support_complaint']:
        ...
        elif step in ['captcha', 'phone', 'email', 'full_name', 'region', 'city_selection',
                      'age', 'company_name', 'contact_person', 'business_activity']:
            # Регистрация - возврат в главное меню
            if self.auth_handlers and self.auth_handlers.role:
                self.auth_handlers.role.cancel_registration(chat_id, user_id, "Регистрация отменена")
            else:
                self.bot.send_message(
                    chat_id,
